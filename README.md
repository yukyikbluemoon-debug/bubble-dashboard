...
แก้ไฟล์ `data/bubble-data.json` แต่ละตัวชี้วัดมี field `value` (ตัวเลขปัจจุบัน), `asOf` (วันที่/แหล่งข้อมูลของตัวเลขนั้น แยกจาก `lastUpdated` ของทั้งไฟล์) และ `riskScore` (0-100 ให้คุณประเมินเองตามตำแหน่งเทียบ scaleMin/scaleMid/scaleMax) — คะแนนรวมบนเกจคำนวณจากค่าเฉลี่ย `riskScore` ของทุกตัวอัตโนมัติ **จุดล่าสุดใน `historicalComposite` ควรอัปเดตให้ตรงกับค่าเฉลี่ยนี้ทุกครั้งเพื่อไม่ให้กราฟกับเกจขัดกันเอง**
